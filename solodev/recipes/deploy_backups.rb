...
script "install_mongo" do
	not_if { ::File.exists?("/root/mongorestore.sh") }
	interpreter "bash"
	user "root"
	cwd "/root"
	code <<-EOH

	touch /root/mongorestore.sh
	if((#{MongoHost} == "instance['private_ip']")); then
		echo "mongorestore --host \`mongo --quiet --eval \"db.isMaster()['primary']\"\` #{mount_path}/mongodumps > /root/mongorestore.log &" >> /root/mongorestore.sh
		echo "/root/heal_mongo.sh > /root/restore.log &" >> /root/mongorestore.sh
	else
		echo "mongorestore #{mount_path}/mongodumps > /root/restore.log &" >> /root/mongorestore.sh
		echo "rm -Rf #{mount_path}/stackname.txt && rm -Rf #{mount_path}/mongohosts.txt" >> /root/mongorestore.sh
	fi
	chmod 700 /root/mongorestore.sh

	mkdir -p #{mount_path}/mongodumps
	touch /root/mongobackup.sh
	if((#{MongoHost} == "instance['private_ip']")); then
		echo "mongodump --host \`mongo --quiet --eval \"db.isMaster()['primary']\"\` --out #{mount_path}/mongodumps >/dev/null 2>&1" >> /root/mongobackup.sh
	else
		echo "mongodump --out #{mount_path}/mongodumps >/dev/null 2>&1" >> /root/mongobackup.sh
	fi

	chmod 700 /root/mongobackup.sh
	(crontab -l 2>/dev/null; echo "30 3 * * 1-6 /root/mongobackup.sh") | crontab -
